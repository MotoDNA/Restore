...
## 10.1 적는 도중에 글자가 지워지던 것 (2026-09-14)

**겪은 일.** 설정 → 사업자 정보를 적는 도중에 칸이 통째로 비었습니다.
25초마다 도는 새로고침이 `renderAll()` 을 부르고, 그 안의 `renderSet()` 이
**저장된 값을 칸에 다시 써 넣기** 때문입니다. 저장 전 글자는 화면에만 있으니 그대로 사라집니다.

막던 것이 `시트가 열려 있으면 건너뛰기` 하나뿐이었는데, 설정은 시트가 아니라 쪽입니다.

| 고친 것 | |
|---|---|
| `renderLive()` | 저절로 도는 새로고침은 **목록만** 다시 그립니다. 서식(`renderSet`)은 손대지 않습니다 |
| `편집중()` | 어느 칸에 커서가 있으면 새로고침 자체를 건너뜁니다 |

⚠ 화면을 다시 그리는 것이 곧 **저장 안 한 값을 지우는 것**입니다.
서식이 있는 쪽을 자동 새로고침에 넣을 때는 늘 이걸 먼저 생각해야 합니다.

## 10.15 점포를 여럿이 맡습니다 (2026-09-14)

`stores.shared_ids` 는 처음부터 있었고 RLS 도 보고 있었는데(`owner_id = auth.uid() or auth.uid() = any(shared_ids)`)
화면에 고를 자리가 없었습니다. 점포 서식에 **함께 맡는 슈퍼바이저** 목록을 넣었습니다.

- 고른 사람은 그 점포와 **그 점포의 발주**를 봅니다(orders 정책이 stores 를 타고 갑니다)
- 담당 슈퍼바이저는 목록에서 빼고, 저장할 때도 `shared_ids` 에서 지웁니다 — 한 사람이 두 자리를 차지하면 안 됩니다
- Re:Call 의 공유 목록과 같은 얼개. 다만 시트를 따로 띄우지 않고 서식 안에 둡니다

## 10.2 핀 없는 링크는 열리지 않습니다 (2026-09-14)

**겪은 일.** 예시 점포를 SQL 로 넣으면서 주석에는 "링크는 꺼 둡니다" 라고 적고
`share_on` 칸을 안 넣었습니다. 그 칸의 기본값이 `true` 라서, **핀 없는 발주 링크
다섯 개가 그대로 살아 있었습니다.** 주소만 알면 점주 이름·연락처·단가가 보이고
발주도 넣을 수 있었습니다. 화면은 그동안 "링크가 새어 나가도 핀을 모르면 못 들어옵니다"
라고 말하고 있었습니다.

막은 자리 셋 — **말이 아니라 칸으로** 못 박습니다.

| 어디 | 무엇 |
|---|---|
| `store-gate` (진짜 자물쇠) | `!s.pin` 이면 403. 핀이 없는 점포는 아예 안 엽니다 |
| 앱 `link-toggle` | 링크를 켤 때 핀이 없으면 그 자리에서 여섯 자리를 만듭니다 |
| 화면 | 핀이 "없음" 이면 빨갛게, 링크가 켜져 있으면 경고 띠 |
| 씨앗 SQL | `pin` · `share_on` 을 칸으로 직접 넣습니다 |

## 10.3 발주 단추는 아래에 붙어 있습니다 (2026-09-14)

품목이 많으면 표가 길어져 단추가 화면 밖으로 나갑니다. 확정 한 번 하려고
끝까지 내렸다가 다시 올라와야 했습니다. `.btnrow.fixrow` 가 `position:sticky; bottom:0` 으로 붙습니다 —
위쪽 막대(`.dbar`)와 같은 얼개입니다(시트가 스크롤되는 통).

⚠ 음수 `margin` 은 `.sheetpage .inner` 의 좌우 여백과 **같아야** 합니다(폰 20px · PC 28px).
어긋나면 띠가 짧게 잘립니다.

⚠ 띠가 있는 시트는 `.inner.tight` 로 아래 빈 자리를 130px → 24px 로 줄입니다.
130px 은 하단 탭을 비켜 두려던 것인데 시트가 그 탭을 덮고 있습니다.

지금은 **발주 시트에만** 붙습니다. 점포·품목·발주 넣기 시트에도 같은 class 를 달면 됩니다.

## 10.4 되돌리기 — 앞으로만 가지 않습니다 (2026-09-14)

확정을 잘못 눌러도 빠져나올 길이 있어야 합니다. 한 발 앞으로 되돌립니다.

| 지금 | 단추 | 지우는 날짜 |
|---|---|---|
| 확정 | 접수로 되돌리기 | `confirmed_on` · `shipped_on` · `done_on` |
| 출고 | 확정으로 되돌리기 | `shipped_on` · `done_on` |
| 완료 | 출고로 되돌리기 | `done_on` |
| 취소 | 접수로 되살리기 | 셋 다 |

⚠ **지나온 날짜를 반드시 지웁니다.** 출고로 되돌렸는데 완료일이 남아 있으면
기록이 "완료됐는데 출고 중" 이라고 말합니다.

⚠ `billed_ym` 이 있는(정산 마감한) 발주는 물음에 그 사실을 먼저 붙입니다 —
청구서가 나간 뒤에 수량이 바뀌면 금액이 달라집니다. 직원은 아예 못 누릅니다(`locked`).

`ship_qty` 는 지우지 않습니다. 되돌린 뒤에도 고쳐 쓸 값입니다.

## 10.5 출고 수량 — 고치는 즉시 금액이 바뀝니다 (2026-09-14)

확정·출고 단계에서 출고 칸을 고치면 **저장하기 전에** 줄 금액 · 면세/과세 공급가액 ·
부가세 · 합계가 다시 그려집니다(`repriceShip()`).

| 지키는 것 | 왜 |
|---|---|
| 세는 규칙은 `calcOrder()` 하나 | 화면 값으로 발주를 흉내 내어 그 함수에 넘깁니다. 여기서 또 세면 언젠가 두 숫자가 갈립니다 |
| 서버에는 안 씁니다 | 한 자 칠 때마다 저장하면 `22` 를 치는 동안 `2` 가 먼저 저장됩니다 |
| 단추가 "아직 저장 안 됨" 으로 | 숫자만 바뀌면 저장한 줄 압니다 |
| 모자람은 빨강, 더 나감은 파랑 | `수량차이(나감, 주문)` 하나가 세 화면(본사 목록·본사 상세·가맹점)의 색을 정합니다 |
| 면세 줄은 면세 품목이 있으면 늘 그립니다 | 0 으로 고칠 때 줄이 사라졌다 나타나면 표가 덜컥거립니다 |

⚠ 발주 줄 금액은 `.lamt` 입니다. `.amt` 는 발주 목록·품목 화면이 이미 쓰고 있어
문서 전체에서 고르면 엉뚱한 숫자를 갈아 끼웁니다.
...
## 11.5 발주 링크와 핀 — 2026-09-01 에 튼튼하게 했습니다

가맹점은 계정이 없습니다. 링크(128비트 토큰)와 **핀**이 전부입니다.
링크가 새어 나가면 마지막으로 남는 벽이 핀 하나입니다.

| | |
|---|---|
| 틀린 횟수 | **`pin_tries` 표**(`sql/0004`)에 셉니다. 10번 틀리면 10분 잠김 |
| 자릿수 | **여섯 자리**(백만 개). `mkPin()` 이 만들어 줍니다 |
| 비교 | `sameSecret()` — 한 글자씩 다 비교합니다. 몇 번째에서 틀렸는지를 걸린 시간으로 못 알아내게 |

### ⚠ 틀린 횟수를 다시 메모리로 옮기지 마세요

예전에는 `store-gate` 함수 안의 `Map` 에 셌습니다. Edge Function 은 잠깐
쉬면 내려갔다 새로 뜨는데, 그때 센 것이 **0 으로 돌아갑니다.** 여러 대가
동시에 돌면 각자 따로 세고, 5000개가 넘으면 통째로 비우기까지 했습니다.
그래서 "10번 틀리면 잠김" 이 실제로는 잘 안 걸렸습니다.

### 네 자리 핀이 남아 있으면

점포 화면에 **빨간 알림**이 뜹니다. 자동으로 안 바꾸는 이유는, 바꾸는
그 순간 점주가 못 들어오기 때문입니다. 본사가 새 핀을 알려 줄 준비가
됐을 때 `핀 새로` 를 직접 눌러야 합니다.
...
| | |
|---|---|
| 핀 세는 자리 | **`pin_tries` 표.** 메모리(Map)로 되돌리면 함수가 새로 뜰 때마다 잊어버려 잠금이 안 걸립니다 |
| 새 핀 자릿수 | `mkPin()` 은 **여섯 자리**입니다. 네 자리는 만 개뿐이라 시간을 들이면 뚫립니다 |
